...
"""Classes and methods to parse data for input to ML tools. Taken from ADAPT Repository
"""
__author__ = 'Hayden Metsky <hayden@mit.edu>'

SCRIPT_PATH = os.path.dirname(os.path.abspath("ADAPT/"))
...
class Cas13ActivityParser:
    ...
    INPUT_TSV_RESAMPLED = os.path.join(SCRIPT_PATH,
            'CCF_merged_pairs_annotated.curated.resampled.tsv.gz')
    INPUT_TSV_MEDIAN = os.path.join(SCRIPT_PATH,
            'CCF_merged_pairs_annotated.curated.median.tsv.gz')
    ...
    def __init__(self, subset=None, context_nt=10, split=(0.8, 0.1, 0.1),
            shuffle_seed=1, stratify_randomly=False, stratify_by_pos=True,
            use_median_measurement=False, classify=True, regress_on_all= False, regress_only_on_active =False ):
        """
        Args:
        ...
        self.classify_activity = classify
    ...
    def read(self):
        ...
        for row in rows:
            ...
            input_feats_key = np.array(input_feats, dtype='f').tobytes()
    ...
    def pos_for_input(self, x):
        ...
        x_key = np.array(x, dtype='f').tobytes()
    ...
    def unique_sampling_idx(self, num_unique_sample, xx):
        ...
        def key(x):
            return np.array(x, dtype='f').tobytes()
...
class Cas13ActivityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.classify:
            return torch.tensor(self.inputs[idx]), torch.tensor(self.targets[idx].item()).long()
        else:
            return torch.tensor(self.inputs[idx]), torch.tensor(self.targets[idx].item()).float()
